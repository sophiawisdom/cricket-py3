...
class DataTable(Sequence, Sized):
    ...
    def __getitem__(self, i):
        "Get the *i*-th inserted value."
        return self._values[i]
       
    def __iter__(self):
        "Returns an iterable of the values in this data table."
        return iter(self._values)
        
    def __len__(self):
        "Returns the number of values (the length) of this data table."
        return len(self._values)
        
    def __bool__(self):
        "Returns whether this data table is not empty."
    ...
    def append(self, value, **columns):
        ...
        for colName, key in list(columns.items()):
...
#                keys[key] = [v for v in keys[key] if v not in values]
    ...
    def column(self, columnName):
        ...
        if isUnique:
            ...
            return ((key, value) for key, lst in list(col.items()) for value in lst)
        
                
    @property
    def values(self):
        '''Return a list of values (without copying) in this data table.'''
        return self._values
    ...
    def setColumnUnique(self, columnName, isUnique):
        ...
        if res[0] != isUnique:
            res[0] = isUnique
            if isUnique:
                res[1] = dict((key, value[0]) for key, value in list(res[1].items()))
            else:
                res[1] = dict((key, [value]) for key, value in list(res[1].items()))
...
if __name__ == '__main__':
    ...
    assert 'w7' not in list(map(itemgetter(1), dt.column('color')))
    dt.associate('w7', 'color', ['red', 'green', 'yellow'])
    assert 'w7' in dt
    assert dt.all('sides', 7) == ['w7']
    assert 'w7' in list(map(itemgetter(1), dt.column('color')))
