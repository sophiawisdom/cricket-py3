...
def _bind(machO, size, symbols):
	...
	while f.tell() < end:
		...
		addr = addr & (0xffffffffffffffff if machO.is64bit else 0xffffffff) # FIXME: how about 64-bit?
...
def _recursiveProcessExportTrieNode(f, start, cur, end, prefix, symbols, macho):
	if cur < end:
		f.seek(cur)
		...
		if termSize:
			sym = prefix
			readULeb128(f)
			file_addr = readULeb128(f)
			vm_addr = macho.mappings.toVM(file_addr)
			symbols.append(Symbol(sym, vm_addr, SYMTYPE_GENERIC, extern=True))
		...
		for i in range(childCount):
			...
			lastPos = f.tell()
			_recursiveProcessExportTrieNode(f, start, start + offset, end, prefix + suffix, symbols, macho)
			f.seek(lastPos)
...
class DyldInfoCommand(LoadCommand):
	...
	def analyze(self, machO):
		...
		if exportSize:
			...
			_recursiveProcessExportTrieNode(machO.file, exportOff, exportOff, exportOff + exportSize, "", symbols, machO)
