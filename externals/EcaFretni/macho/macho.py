...
class MachO(object):
    ...
    def __pickArchFromFatFile(self):
        ...
        bestMatch = self._arch.bestMatch(list(offsets.keys()), scoreLimit)
        
        # Cannot find best match, raise an error. 
        if bestMatch is None:
            raise MachOError('Cannot find an arch matching "{}". Available archs are: {}'.format(self._arch, ', '.join(map(str, list(offsets.keys()))) ))
