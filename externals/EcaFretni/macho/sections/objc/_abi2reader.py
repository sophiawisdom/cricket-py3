...
def readIvar(machO):
	...
	offset = machO.deref(offsetPtr, machO.makeStruct('L'))[0]
	name = machO.derefString(namePtr)
	encoding = machO.derefString(encPtr)
	return Ivar(name, encoding, offset, offsetPtr)
...
def classAt(machO, vmaddr, classes):
	...
	if not cls:
		try:
			...
			clsName = readClassName(machO, vmaddr)
			name = '_OBJC_CLASS_$_' + (clsName if clsName else "...")
